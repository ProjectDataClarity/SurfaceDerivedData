   ...
   function stickSlipIndex  = computeStickSlipIndexApproach2(timeInterval_sec, statesData) 
   ...
   This is approach 2 of calculation stickSlipIndex: Shell Internal...
   
   SSI = (RPMmax-RPMmin)/RPMave
   ...
   Other Comments: The 1Hz RPM data is assumed to be raw data collected once every 
  second, without averaging, or any other filtering operation. 
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    timeInterval_sec
         ...
         if(rigStatesForSampleData1(i)==2) 
              % Once it starts drilling wait until after "timeInterval_sec" seconds before calulating a value
               ...
               if(timeIntervalCounter<=timeInterval_sec)
                  ...
                  rpmMax = max(topdriveSpeed_rpm(i-timeInterval_sec:i));
                  rpmMin = min(topdriveSpeed_rpm(i-timeInterval_sec:i));
                  rpmAve = mean(topdriveSpeed_rpm(i-timeInterval_sec:i));
                  if (rpmAve ~= 0)
                      stickSlipIndex(i) = (rpmMax-rpmMin)/rpmAve ; 
                  else
                      stickSlipIndex(i) = 0;
                  end
               end
     ...
     header='Time(sec), Block Height(feet),Flow Out(%),Hookload(klbf),Top Drive Speed(RPM),Strokes Per Minute #1,Strokes Per Minute #2,Standpipe Pressure (psi),Top Drive Torque (ftlb),Survey Inclination (degrees), Survey Azimuth (degrees), Data Instance, Rig State Code,Stick Slip Index (rpm Based)'; 
     ...
     title (sprintf('Stick Slip Index (rpm Based)'));
     xlabel('Data Instance (sec)');
     ylabel('Stick Slip Index (rpm Based)');
